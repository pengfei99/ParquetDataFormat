...
For more details about the parquet data format, please visit [Parquet format explained](https://github.com/pengfei99/ParquetPyArrow/blob/main/README.md)
...
As parquet is a standard, and is implemented by various framework. Some framework does not respect or implemented 100% of the parquet standard. This means one parquet file that are generated by one framework may not be readable by another framework. We did not find an official doc which address this issue completly. As a result, the below list may not be complete. So far, we have found four possible compatibility issues:
